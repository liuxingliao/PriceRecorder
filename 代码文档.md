...
struct ProductEditSheet: View {
                ...
                Section("商品信息") {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        if !filteredNames.isEmpty {
                            suggestionList(filteredNames, selection: $name)
                        }
                    }
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        if !filteredBrands.isEmpty {
                            suggestionList(filteredBrands, selection: Binding(
                                get: { brand ?? "" },
                                set: { brand = $0.isEmpty ? nil : $0 }
                            ))
                        }
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        if !filteredSpecs.isEmpty {
                            suggestionList(filteredSpecs, selection: Binding(
                                get: { spec ?? "" },
                                set: { spec = $0.isEmpty ? nil : $0 }
                            ))
                        }
    ...
    // 建议列表组件（辅助函数）
    private func suggestionList(_ suggestions: ArraySlice<String>, selection: Binding<String>) -> some View {
        VStack(alignment: .leading, spacing: 0) {
            ForEach(Array(suggestions), id: \.self) { suggestion in
                Button(action: {
                    selection.wrappedValue = suggestion
                }) {
                    HStack {
                        Text(suggestion)
                            .foregroundColor(.primary)
                        Spacer()
                        if selection.wrappedValue == suggestion {
...
**ProductDetailView 中的自动建议功能**:

在 `ProductDetailView` 的编辑模式中，同样实现了商品名称、品牌、规格字段的自动建议功能，实现方式与 `ProductEditSheet` 类似。

核心特性:
- 查询所有已有商品记录用于生成建议数据
- 三个计算属性分别提供商品名称、品牌、规格的建议列表
- 每个输入框下方实时显示匹配的建议
- 点击建议可快速填入
- 最多显示 6 条建议，去重处理

关键代码位置: `ProductDetailView.editForm` (第 167-388 行)
